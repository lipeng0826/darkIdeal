...
# ==================== 背景色系 ====================
const BG_BASE := Color(0.96, 0.94, 0.91)           # 米白底色
const BG_CARD := Color(1.0, 0.99, 0.97, 0.95)      # 卡片白
const BG_ELEVATED := Color(1.0, 1.0, 1.0, 0.98)    # 浮起面板
const BG_NAV := Color(0.98, 0.96, 0.93, 0.99)      # 导航栏底
const BG_HEADER := Color(0.94, 0.92, 0.88)         # 顶栏底色
const BG_INPUT := Color(0.95, 0.93, 0.90)          # 输入框底

# ==================== 主题色 ====================
const PRIMARY := Color(0.85, 0.42, 0.48)           # 蔷薇粉（主按钮）
const PRIMARY_LIGHT := Color(0.95, 0.75, 0.78)     # 浅粉
const PRIMARY_DARK := Color(0.65, 0.28, 0.35)      # 深粉
const SECONDARY := Color(0.40, 0.65, 0.85)         # 天蓝
const SECONDARY_LIGHT := Color(0.70, 0.85, 0.95)   # 浅蓝
...
# ==================== 文字色 ====================
const TXT_PRIMARY := Color(0.22, 0.22, 0.28)       # 主文字
const TXT_SECONDARY := Color(0.50, 0.48, 0.55)     # 次要文字
const TXT_DISABLED := Color(0.72, 0.70, 0.74)      # 禁用文字
...
static func make_card(accent: Color = Color.TRANSPARENT) -> StyleBoxFlat:
	...
	s.shadow_color = Color(0.0, 0.0, 0.0, 0.06)
	s.shadow_size = 4
...
static func make_header_bg() -> StyleBoxFlat:
	...
	s.shadow_color = Color(0.0, 0.0, 0.0, 0.03)
	s.shadow_size = 2
